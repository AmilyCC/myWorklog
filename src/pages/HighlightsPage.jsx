          ...
          return (
            <div key={i} className="bg-white rounded-2xl border border-slate-200 border-l-4 border-l-accent-400 p-5 shadow-sm">
              {isEditing ? (
                ...
                <>
                  <div className="flex items-start justify-between gap-2 border-b-2 border-accent-300 pb-1 mb-3">
                    <p className="text-sm font-bold text-accent-700">{h.category || '未分類'}</p>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-xs text-primary-400 font-medium">{h.date}</span>
                      <button onClick={() => startEdit(i, h)} title="編輯"
                      ...
                      <button onClick={() => deleteHighlight(i)} title="刪除"
                        className="p-1.5 rounded-lg text-slate-400 hover:text-accent-600 hover:bg-accent-50 transition">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
